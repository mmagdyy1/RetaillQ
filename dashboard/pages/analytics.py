...
def inject_css():
    ...
    <style>
      ...
      .kpi-label {{ font-size:.8rem; color:
      .kpi-value {{ font-size:2rem; font-weight:800; color:
      .kpi-sub   {{ font-size:.75rem; color:
      ...
      .freshness-bar {{
        ...
        border:1px solid 
        gap:10px; font-size:.85rem; color:
      }}
      ...
      .badge-jumia  {{ background:#FF6B3520; color:
      .badge-noon   {{ background:#FECC0020; color:
      .badge-amazon {{ background:#00A8E120; color:
...
def show():
    ...
    if not trends_f.empty:
        ...
        fig_gap.add_trace(go.Bar(
            ...
            marker=dict(
                color=gap["gap_pct"],
                colorscale=[[0, "#2ecc71"], [0.5, "#f39c12"], [1, "
                showscale=False,
            ),
    ...
    if not trends_f.empty:
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(x=cat_agg["category"], y=cat_agg["min_price"], name="Min",  marker_color="
        fig2.add_trace(go.Bar(x=cat_agg["category"], y=cat_agg["avg_price"], name="Avg",  marker_color=ACCENT))
        fig2.add_trace(go.Bar(x=cat_agg["category"], y=cat_agg["max_price"], name="Max",  marker_color="
        fig2.update_layout(
    ...
    if not anomalies.empty:
        ...
        with col_a:
            ...
            fig_sev = px.pie(
                ...
                color="severity",
                color_discrete_map={"HIGH": "#e74c3c", "MEDIUM": "
